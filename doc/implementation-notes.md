# Implementation Notes - Heat Transfer
...
```
main.ts
  └─ TemperatureScreen             (Screen<TemperatureModel, TemperatureScreenView>)
       ├─ TemperatureModel          state + logic  (src/temperature/model/)  ← stub: add physics here
       └─ TemperatureScreenView     visuals        (src/temperature/view/)
            ├─ TemperatureScreenSummaryContent     (PDOM overview — reference a11y pattern)
            └─ TemperatureKeyboardHelpContent      (keyboard help dialog)

src/common/
  ├─ HeatTransferPanel.ts           pre-themed panel (uses HeatTransferColors)
  ├─ HeatTransferButtonOptions.ts   flat button / combo-box option bundles
  └─ TimeModel.ts          composable play/pause + elapsed time

src/preferences/
  ├─ HeatTransferPreferencesModel   sim-specific pref state
  ├─ HeatTransferPreferencesNode    pref UI in Preferences → Simulation
  └─ heatTransferQueryParameters    QueryStringMachine declarations
```
...
Every control panel should use `HeatTransferPanel` so projector-mode switching is automatic:

```typescript
import { HeatTransferPanel } from "../../common/HeatTransferPanel.js";
const panel = new HeatTransferPanel(content);
const panelWide = new HeatTransferPanel(content, { xMargin: 20 });
```
...
Use `HEAT_TRANSFER_COMBO_BOX_OPTIONS` + `LIGHT_SURFACE_TEXT_FILL` for light control surfaces on dark panels.
...
- `TemperatureScreenSummaryContent` with a live `currentDetailsContent` `DerivedProperty` over model state.
- Explicit `pdomOrder` + `TemperatureKeyboardHelpContent`.
- Strings under `a11y` in locale JSON → `StringManager.getTemperatureA11yStrings()`.
...
`src/temperature/`, `StringManager` screen-name getters, optional shared root model, a shared
`src/common/HeatTransferScreenIcons.ts` module (`create{Screen}Icon()` factories wired as
...
- `TemperatureModel.step()` / `reset()` — empty placeholders until you add physics.
- Placeholder play-area content in `TemperatureScreenView` — replace with real UI.
