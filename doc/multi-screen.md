...
This sim was scaffolded by `npm run scaffold-screens` into `src/temperature/`, `src/conduction/`, `src/convection/`, `src/combined/`, `src/materials/` (fleet naming:
...
1. Copies the `temperature/` prototype into `src/<kebab>/` per screen
...
4. Removes the prototype `temperature/` folder
...
main.ts
  └─ TemperatureScreen            (Screen<TemperatureModel, TemperatureScreenView>)
       ├─ TemperatureModel         owns all state
       └─ TemperatureScreenView    owns all visuals
```
...
template's `TemperatureScreen.ts`. Screen icons live in one shared module under
...
```
src/common/HeatTransferScreenIcons.ts
```
